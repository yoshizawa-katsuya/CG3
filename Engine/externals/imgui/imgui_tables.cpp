...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        // DrawParticle cell background color
        ...
        // DrawParticle top border
        if (border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y1), ImVec2(table->BorderX2, bg_y1), border_col, border_size);

        // DrawParticle bottom border at the row unfreezing mark (always strong)
...
// DrawParticle channel allocation (before merging):
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    // DrawParticle inner border and resizing feedback
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    const float border_size = TABLE_BORDER_SIZE;
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // DrawParticle in outer window so right-most column won't be clipped
    ...
    {
        // DrawParticle bottom-most row border
...
void ImGui::EndColumns()
{
    ...
    // DrawParticle columns borders and handle resize
